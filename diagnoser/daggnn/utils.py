...
def simulate_random_dag(d: int, degree: float, graph_type: str, w_range: tuple = (0.5, 2.0)) -> nx.DiGraph:
...
def simulate_sem(
    G: nx.DiGraph, n: int, x_dims: int, sem_type: str, linear_type: str, noise_scale: float = 1.0
) -> np.ndarray:
    ...
    for j in ordered_vertices:
        ...
        if sem_type == "linear-gauss":
            if linear_type == "linear":
                ...
                X[:, j, 0] = 2.0 * np.sin(eta) + eta + np.random.normal(scale=noise_scale, size=n)
...
def count_accuracy(G_true: nx.DiGraph, G: nx.DiGraph, G_und: nx.DiGraph = None) -> tuple:
...
def get_buckets(dist, num_buckets):
    ...
    for i in range(num_buckets):
        if i < num_buckets - 1:
            idx = np.where(np.all(np.vstack((dist > thresholds[i], dist <= thresholds[i + 1])), 0))[0]
        else:
