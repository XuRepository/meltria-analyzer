...
def detect_sigma(samples: np.ndarray, sigma: int, chaos_duration_min=5) -> BkpsStatus:
    ...
    minus, plus = (
        samples.mean() - sigma * samples.std(),
        samples.mean() + sigma * samples.std(),
    )
    chaos_plots: int = chaos_duration_min * 60 // TIME_INTERVAL_SEC + 1
    chaos_injected_pt: int = len(samples) - chaos_plots
...
def main():
    ...
    parser.add_argument("metricsfiles", nargs="+", help="metrics output JSON file")
    parser.add_argument("--out", help="output file path")
    parser.add_argument("--out-format", choices=["json", "csv"], default="json", help="output format")
    parser.add_argument(
        "--ad-method",
