...
from diagnoser import call_graph, causalrca, pyrca, rcd
...
def build_causal_graph_with_pcalg(
    ...
    if disable_orientation:
        ...
        return call_graph.fix_edge_directions_in_causal_graph(G, pk)
...
def build_causal_graphs_with_pgmpy(
    ...
    if disable_orientation:
        ...
        G = result
        G = call_graph.fix_edge_directions_in_causal_graph(G, pk)
    return G
...
def build_causal_graphs_with_cdt(
    ...
    return G if disable_orientation else call_graph.fix_edge_directions_in_causal_graph(G, pk)
...
def build_causal_graphs_with_causallearn(
    ...
    init_dg = call_graph.fix_edge_directions_in_causal_graph(init_g, pk)
...
def build_causal_graph_with_lingam(
    df: pd.DataFrame,
    nodes: mn.MetricNodes,
    init_g: nx.Graph,
    pk: PriorKnowledge,
    ...
    for u, v in combinations(init_g.nodes, 2):
        if not init_g.has_edge(u, v):
            prior_knowledge[mapping[u], mapping[v]] = 0
        if not init_g.has_edge(v, u):
    ...
    assert model.adjacency_matrix_ is not None
    # Apply abs because the connection strength generated by lingam has a possibly negative value.
    adjacency_ = pd.DataFrame(
        np.abs(model.adjacency_matrix_),
...
def build_causal_graph_without_citest(
    ...
    return (
        G
        if disable_orientation
        else call_graph.fix_edge_directions_in_causal_graph(G.to_directed(), pk)
    )
...
def build_causal_graph_with_library(
    ...
    **kwargs: Any,
    ...
    match (pc_library):
        ...
        case "lingam":
            G = build_causal_graph_with_lingam(
                df=dataset,
                nodes=nodes,
                init_g=init_g,
                pk=pk,
...
def build_causal_graph(
    ...
    **kwargs: Any,
    ...
    init_g: nx.Graph = call_graph.prepare_init_graph(
        nodes, pk, enable_prior_knowledge=enable_prior_knowledge
    )

    if use_call_graph or use_complete_graph:
        G = call_graph.fix_edge_directions_in_causal_graph(init_g, pk)
    else:
