...
def check_causal_path_based_validity(record: DatasetRecord, anomalies: pd.Series) -> bool:
    ...
    gt_metrics_routes = select_ground_truth_metrics_in_routes(
        record.pk, anomaly_metrics, record.chaos_type(), record.chaos_comp(),
    )
...
def detect_anomalies_in_record(
    ...
    df = pd.DataFrame(
        items,
        columns=['target_app', 'chaos_type', 'chaos_comp', 'chaos_case_num', 'n_sigma', 'valid_dataset_ok', 'num_anomalies', 'anomalies_rate'],
