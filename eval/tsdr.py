...
def sweep_tsdr_and_save_as_cache(
    ...
    for i, (tsdr_options, metric_types, _use_manually_selected_metrics, time_range, sampling_scale_factor) in enumerate(combinations, 1):
        if resuming_no > i:
            continue
        tqdm.write(
            f"{i}/{len(combinations)}: Starting experiment {experiment_id} with {metric_types}, {tsdr_options}, {time_range}, sampling_scale_factor={sampling_scale_factor}, use_manually_selected_metrics={_use_manually_selected_metrics}"
        )
